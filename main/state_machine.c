...
static void handle_button_event(button_id_t btn, button_event_t evt)
{
    ...
    if (s_state == STATE_READING)
    {
        if (btn == BTN_READING)
            return; /* 重复按无响应 */
        if (btn == BTN_NIGHT && evt == BTN_EVT_SHORT_PRESS)
        {                      // BTN_NIGHT + 短按: 切换到夜灯模式
    ...
    if (s_state == STATE_NIGHT)
    {
        if (btn == BTN_NIGHT)
            return; /* 重复按无响应 */
        if (btn == BTN_READING && evt == BTN_EVT_SHORT_PRESS)
        {                       // BTN_READING + 短按: 切换到阅读模式
    ...
    {
        if (btn == BTN_POWER && evt == BTN_EVT_SHORT_PRESS)
        { // BTN_POWER + 短按: 开机
            do_power_on();
        }
...
static void handle_command(command_id_t cmd, lamp_id_t lamp, int32_t value)
{
    ...
    case CMD_SET_MODE:
        ...
        case 1: /* reading */
            ...
            {
                if (s_state != STATE_NORMAL)
                    do_exit_special(); // 先退出夜灯模式，恢复快照到 NORMAL
        ...
        case 2: /* night */
            ...
            {
                if (s_state != STATE_NORMAL)
                    do_exit_special(); // 先退出阅读，恢复快照到 NORMAL
